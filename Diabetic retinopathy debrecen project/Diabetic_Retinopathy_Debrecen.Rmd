...
L'ensemble des données contient des caractéristiques extraites des images _Messidor_ dans le but de prédire si une personne est atteinte de rétinopathie diabétique. Toutes les caractéristiques représentent soit une lésion détectée, soit une caratéristique descriptive d'une partie anatomique, soit un descripteur au niveau de l'image. 
...
str(data)
```
Il n'y a pas de valeurs nulle dans notre dataset. De plus chaque variable possède désormais le bon type. Nous allons pouvoir procéder à l'analyse préliminaire de nos données. 
...
library(DiagrammeR)
xgb.plot.importance(xgb.importance(boostedtree, feature_names = colnames(X_data)))
```
